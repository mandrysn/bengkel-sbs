    ...
    <body>
          ...
          <div class="col-md-6">
            ...
            <table class="table" width="50%">
                <tr><th class="left" width="30%">Nomor Barang Keluar Pribadi</th><td width="1%">:</td><td>{{ $sot->bbk_material }}</td></tr>
                <tr><th class="left">Tanggal Barang Keluar</th><td>:</td><td>{{ $sot->tanggal_masuk }}</td></tr>
            </table>
        </div>
            ...
            <table class="table table-bordered table-hover">
                <thead>
                    <tr>
                        ...
                        <th class="border center">Biaya per Transaksi (Rp)</th>
                        <th class="border center" >Diskon (%)</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="border">{{ $data->barang->nama_barang }} / {{ $data->barang->kode_barang }}</td>
                        <td class="border">{{ $data->suplier->nama_suplier }}</td>
                        <td class="border">{{ $data->bk_quantity }} {{ $data->barang->satuan->nama_satuan }}</td>
                        <td class="border">{{ number_format($data->harga_transaksi) }}</td>
                        <td class="border">{{ $data->diskon }}</td>
                        <td class="border">{{ number_format( $tagihan = ( $data->harga_transaksi - ($data->harga_transaksi * $data->diskon / 100) ) * $data->bk_quantity) }}</td>
                        <td class="border">{{ $data->keterangan_transaksi }}{{ Form::hidden('total',  $total += $tagihan) }}</td>
                    </tr>
                ...
                <tfoot>
                    <tr>
                        <td colspan="6" class="border" align="right" valign="middle"><h5>total (Rp)</h5></td>
                        <td class="border adleft" valign="middle"><h5>{{ number_format($total) }}</h5></td>
                    </tr>
                </tfoot>
            </table>
