                ...
                <tbody>
                    ...
                    @foreach ($tagihans as $data)
                        <tr>
                            ...
                            <td>
{!! Form::open(['route' => [$route.'.destroy', $data->sid], 'method' => 'delete', 'class' => 'form-inline', 'id' => "delete-form"]) !!}
    {!! link_to_route($route.'.edit', ' Ubah', $data->sid, ['class' => 'btn btn-primary'] ) !!}
    {!! Form::submit('Hapus', ['class' => 'btn btn-danger', 'onclick' => "return confirmation();"]) !!}
	<a href="{{ URL($route . '/' . $data->sid) }}" class="btn btn-success">Detail</a>
{!! Form::close() !!}
                            </td>
                        </tr>
                    @endforeach
