...
<body>
    ...
    <table>
            ...
            @php ($total_or = 0) @php ($total_tagihan = 0)
            ...
            <tr>
                ...
                <td class="adjacent">
                    {{ number_format($or = $data->jumlah_or) }}{{ Form::hidden('or',  $total_or += $or) }}
                </td>
                <td class="adjacent adleft">
                    {{ number_format($tagih = $data->tagihan) }}{{ Form::hidden('tagihan',  $total_tagihan += $tagih) }}
                </td>
            </tr>
            @endforeach
            <tr>
                    <td colspan="7" class="adjacent">total</td>
                    <td class="adjacent">{{ number_format($total_or) }}</td>
                    <td class="adjacent adleft">{{ number_format($total_tagihan) }}</td>
                </tr>
    </table>
</body>
