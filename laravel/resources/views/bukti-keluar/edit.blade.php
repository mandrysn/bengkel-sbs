 ...
 <h1 class="page-header">{{ $title }}</h1>
                ...
                {!! Form::model($order, ['method' => 'PATCH', 'action' => [$controller.'@update', $order->id], 'files' => 'true', 'class' => 'form-horizontal']) !!}
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-lg-3">
                            {{ Form::text('bbk_material', $order->bbk_material, ['class' => 'form-control', 'placeholder' => 'No. Bukti Material Keluar', 'readonly']) }}
                        </div>
                    </div>
                        ...
                        <div class="col-lg-3">
                            <select name="id" class="form-control selectpicker" data-live-search="true" placeholder="Pilih Barang...">
                                ...
                                @foreach ( $barang as $key )
                                    <option value="{{ $key->id }}">{{ $key->suplier->nama_suplier }} - {{ $key->barang->no_part_barang }} - {{ $key->barang->nama_barang }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('id'))
                ...
                <table class="table table-bordered table-hover">
                    ...
                    @foreach($bbk as $index => $data)
                        <tr>
                            ...
                            <td>
                            {{ $data->bk_quantity }} {{ $data->barang->satuan->nama_satuan }}
                            </td>
                            <td>
                            {{ number_format($data->harga_transaksi) }}
                            </td>
                            <td>{{ $data->diskon }}</td>
                            <td>{{ number_format( $tagihan_barang = ( $data->harga_transaksi - ($data->harga_transaksi * $data->diskon / 100) ) * $data->bk_quantity ) }}</td>
                            <td>{{ $data->suplier->nama_suplier }}</td>
                            <td>{{ $data->keterangan_transaksi }} {{ Form::hidden('total',  $total_barang += $tagihan_barang) }}</td>
                            <td width="5%">
                        {!! Form::open(['route' => ['detail-bukti-keluar.destroy', $data->id], 'method' => 'delete', 'class' => 'form-inline', 'id' => "delete-form"]) !!}
