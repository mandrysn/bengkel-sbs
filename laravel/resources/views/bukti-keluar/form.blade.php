<div class="form-group{{ $errors->has('bbk_material') ? ' has-error' : '' }}">
    {{ Form::label('no_barang_keluar', null, ['class' => 'col-lg-2 control-label']) }}
    <div class="col-lg-3">
        {{ Form::text('bbk_material', $kode, ['class' => 'form-control', 'placeholder' => 'No. Bukti Material Kaluar', 'readonly']) }}
        @if($errors->has('bk_material'))
            <span class="help-block">
                <strong>{{ $errors->first('bbk_material') }}</strong>
            </span>
        @endif
    </div>
</div>
